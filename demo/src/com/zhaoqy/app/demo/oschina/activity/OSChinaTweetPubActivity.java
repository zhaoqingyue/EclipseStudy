 ...
 *
 * @Title: OSChinaTweetPubActivity.java
 * @Prject: demo
 * @Package: com.zhaoqy.app.demo.oschina.activity
 * @Description: 发表动弹
 * @author: zhaoqy
 * @date: 2015-11-23 上午11:14:54
 * @version: V1.0
 */
...
import com.zhaoqy.app.demo.oschina.util.MediaUtils;
...
public class OSChinaTweetPubActivity extends OSChinaBaseActivity implements OnClickListener, OnLongClickListener, OnItemClickListener
{
	...
	private String       mTweetKey = AppConfig.TEMP_TWEET;
	private String       mTweetImageKey = AppConfig.TEMP_TWEET_IMAGE;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) 
	{
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_oschina_tweet_pub);
		mContext = this;
    ...
    {    	
    	...
    	mSoft = (ImageView)findViewById(R.id.tweet_pub_footbar_software);
    	mClear = (LinearLayout)findViewById(R.id.tweet_pub_clearwords);
    ...
    private void initData() 
    {
    	...
    	if(atuid > 0)
    	{
    		mTweetKey = AppConfig.TEMP_TWEET + "_" + atuid;
    		mTweetImageKey = AppConfig.TEMP_TWEET_IMAGE + "_" + atuid;
    	}
    	...
    	{		
			...
			{
				//保存当前EditText正在编辑的内容
				AppProperty.setProperty(mContext, mTweetKey, s.toString());
				//显示剩余可输入的字数
    	...
    	//显示临时编辑内容
		UIHelper.showTempEditContent(mContext, mContent, mTweetKey);
		//显示临时保存图片
		String tempImage = AppProperty.getProperty(mContext, mTweetImageKey);
		if(!StringUtils.isEmpty(tempImage)) 
		{
    		Bitmap bitmap = ImageUtils.loadImgThumbnail(tempImage, 100, 100);
    		if(bitmap != null) 
    		{
    			mImgFile = new File(tempImage);
		...
		if(atuid > 0 && mContent.getText().length() == 0)
		{
			...
			//设置光标位置
    		mContent.setSelection(atme.length());
		}
	}
	...
	public void onClick(View v) 
	{
		...
		case R.id.tweet_pub_publish:
		{
			...
			mTweet = new Tweet();
			mTweet.setAuthorId(UserHelper.getLoginUid());
			mTweet.setBody(content);
			mTweet.setImageFile(mImgFile);
			...
			final Handler handler = new Handler()
			{
				public void handleMessage(Message msg) 
				{
					...
					{
						//清除之前保存的编辑内容
						AppProperty.removeProperty(mContext, mTweetKey,mTweetImageKey);						
						finish();
					}
					else
					{
			...
			new Thread()
			{
				public void run() 
				{
					...
					try 
					{
						res = UserHelper.pubTweet(mContext, mTweet);
						what = 1;
						msg.what = 1;
						msg.obj = res;
		            } 
					...
					UIHelper.sendBroadCastTweet(mContext, what, res, mTweet);
				}
			}.start();
			break;
		}
		case R.id.tweet_pub_footbar_face:
		{
			showOrHideIMM();
			break;
		}
		case R.id.tweet_pub_footbar_photo:
		{
			//隐藏软键盘
			mInputMethodManager.hideSoftInputFromWindow(v.getWindowToken(), 0);  
			//隐藏表情
		...
		case R.id.tweet_pub_footbar_atme:
		{
			//显示软键盘
			showIMM();			
    		//在光标所在处插入“@用户名”
		...
		case R.id.tweet_pub_footbar_software:
		{
			//显示软键盘
			showIMM();
	...
	public boolean onLongClick(View v) 
	{
		...
		.setPositiveButton(R.string.sure, new DialogInterface.OnClickListener() 
		{
			...
			{
				//清除之前保存的编辑图片
				AppProperty.removeProperty(mContext, mTweetImageKey);
				mImgFile = null;
    ...
    private void showOrHideIMM() 
    {
    	...
    	{
			//隐藏软键盘
			mInputMethodManager.hideSoftInputFromWindow(mContent.getWindowToken(), 0);
			//显示表情
			showFace();				
		}
    	else
    	{
			//显示软键盘
			mInputMethodManager.showSoftInput(mContent, 0);
			//隐藏表情
			hideFace();
		}
    }
		...
		setItems(items, new DialogInterface.OnClickListener()
		{
			public void onClick(DialogInterface dialog, int item) 
			{
				...
				else if (item == 1) 
				{
					...
					{
						UIHelper.ToastMessage(mContext, "无法保存照片，请检查SD卡是否挂载");
						return;
					}
		...
		new Thread()
		{
			public void run() 
			{
		        ...
		        {         	
		        	...
		        	{
		        		mLarge = ImageUtils.getAbsoluteImagePath(OSChinaTweetPubActivity.this,thisUri);
		        	}
		        	else
		        	{
		        		mLarge = thePath;
		        	...
		        	String attFormat = FileUtils.getFileFormat(mLarge);
		    		...
		    		if(MethodsCompat.isMethodsCompat(android.os.Build.VERSION_CODES.ECLAIR_MR1))
		    		{
		    			String imgName = FileUtils.getFileName(mLarge);
		    			bitmap = ImageUtils.loadImgThumbnail(OSChinaTweetPubActivity.this, imgName, MediaStore.Images.Thumbnails.MICRO_KIND);
		    		}
		        	
		        	if(bitmap == null && !StringUtils.isEmpty(mLarge))
		        	{
		        		bitmap = ImageUtils.loadImgThumbnail(mLarge, 100, 100);
		        	}
		        }
				...
				if(bitmap!=null)
				{
					...
					String largeFileName = FileUtils.getFileName(mLarge);
					String largeFilePath = savePath + largeFileName;
					//判断是否已存在缩略图
					if(largeFileName.startsWith("thumb_") && new File(largeFilePath).exists()) 
					{
						mThumbnail = largeFilePath;
						mImgFile = new File(mThumbnail);
					} 
					else 
					{
						//生成上传的800宽度图片
						String thumbFileName = "thumb_" + largeFileName;
						mThumbnail = savePath + thumbFileName;
						if(new File(mThumbnail).exists())
						{
							mImgFile = new File(mThumbnail);
						}
						else
						{
							...
							{
								//压缩上传的图片
								ImageUtils.createImageThumbnail(mContext, mLarge, mThumbnail, 800, 80);
								mImgFile = new File(mThumbnail);
							}
							catch (IOException e) 
							{
					...
					//保存动弹临时图片
					AppProperty.setProperty(mContext, mTweetImageKey, mThumbnail);
					Message msg = new Message();
					msg.what = 1;
					msg.obj = bitmap;
					handler.sendMessage(msg);
